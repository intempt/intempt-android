 ...
 *   - package moved to com.intempt.core.queue
 *   - replaced form-urlencoded body encoding with Intempt's raw JSON contract
 *   - added Basic authorization header support
 *   - replaced the 4xx-discard error taxonomy so 429 is retried
 */
...
public class HttpService implements RemoteService {
    ...
    public HttpService(
            boolean shouldGzipRequestPayload, NetworkErrorListener networkErrorListener) {
    ...
    void checkIsServerBlockedSync() {
        ...
        try {
            ...
            if (backupBlocked) {
                QueueLog.v(LOGTAG, "AdBlocker is enabled. " + errorMsg);
                onNetworkError(null, primaryHost, primaryInet.getHostAddress(),
                        startTimeNanos, -1, -1, new IOException(errorMsg));
            } else {
                QueueLog.v(LOGTAG, "Primary host blocked, but backup host is available.");
            }
        } catch (Exception e) {
            QueueLog.v(LOGTAG, "Primary server blocked-check failed, not assuming blocked", e);
        }
    }
    ...
    public boolean isOnline(Context context, OfflineMode offlineMode) {
        ...
        try {
            ...
            isOnline = true;
            QueueLog.v(LOGTAG, "Don't have permission to check connectivity, will assume we are online");
        }
        return isOnline;
    }
    ...
    public RequestResult performRequest(
        ...
        while (retries < 3) {
            ...
            if (mBackupHost != null && !mBackupHost.isEmpty()) {
                ...
                if (backupUrl.equals(endpointUrl)) {
                    ...
                    QueueLog.w(LOGTAG, "Failed to replace host for backup, skipping backup attempt");
                } else {
                    QueueLog.v(LOGTAG, "Primary failed, trying backup: " + backupUrl);
                    ...
                    QueueLog.w(LOGTAG, "Backup also failed: " + backupResult.exception.getMessage());
                }
            }
            ...
            retries++;
            if (retries < 3) {
                QueueLog.d(LOGTAG, "Attempt " + retries + " failed, retrying...");
        ...
        QueueLog.e(LOGTAG, method + " request to " + endpointUrl + " failed after " + retries + " attempts");
    ...
    String replaceHost(String url, String newHost) {
        try {
            ...
            QueueLog.e(LOGTAG, "Failed to replace host", e);
            return url;
        }
    }
    ...
    private InternalRequestResult tryRequestWithHost(
        ...
        try {
            ...
            if (response == null) {
                QueueLog.v(LOGTAG, hostLabel + " request returned null response");
            ...
            QueueLog.w(LOGTAG, "Client error from " + hostLabel + " host, not attempting backup: " + e.getMessage());
            return InternalRequestResult.failure(e, true, url);
        } catch (IOException e) {
            QueueLog.v(LOGTAG, hostLabel + " request failed: " + e.getMessage());
            return InternalRequestResult.failure(e, false, url);
        } catch (Exception e) {
            QueueLog.v(LOGTAG, hostLabel + " request failed with exception: " + e.getMessage());
            return InternalRequestResult.failure(e, false, url);
        }
    }
    ...
    private byte[] performSingleRequest(
        ...
        try {
                ...
                try { // Get IP Address for error reporting, but don't fail request if DNS fails here
                    ...
                    QueueLog.v(LOGTAG, "Could not resolve IP address for " + url.getHost(), e);
                ...
                if (method == HttpMethod.POST) {
                    ...
                    if (requestBodyBytes != null) {
                        ...
                        QueueLog.v(LOGTAG, "Sending raw body of size: " + uncompressedBodySize);
                    } else if (params != null) {
                        ...
                        QueueLog.v(LOGTAG, "Sending URL params (raw size): " + uncompressedBodySize);
                        ...
                        if (shouldGzipRequestPayload) {
                            ...
                            QueueLog.v(LOGTAG, "Gzipping params, compressed size: " + compressedBodySize);
                        } else {
                        ...
                        QueueLog.v(LOGTAG, "Sending POST request with empty body.");
                    }
                ...
                QueueLog.v(LOGTAG, "Response Code: " + responseCode);
                if (interactor != null && isProxyRequest(endpointUrl)) {
                ...
                if (responseCode >= 200 && responseCode < 300) { // Success
                    ...
                    try {
                        ...
                        if (in != null) {
                            ...
                            QueueLog.w(LOGTAG, "Error Body: " + errorBody);
                        }
                    } catch (Exception e) {
                        QueueLog.w(LOGTAG, "Could not read error stream.", e);
                    }
                    // Report error via listener
                    onNetworkError(
                            connection,
        ...
        if (this.networkErrorListener != null) {
            ...
            if (connection != null) {
                try {
                    ...
                    QueueLog.w(LOGTAG, "Could not retrieve response code/message after error", respExc);
                }
            }
            ...
            try {
                ...
                QueueLog.e(LOGTAG, "Network error listener threw an exception", listenerException);
            }
        }
    }
    ...
    private static final String LOGTAG = "Intempt.Message";
